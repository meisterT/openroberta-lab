...
public class Ev3C4ev3CompilerWorkflow extends AbstractCompilerWorkflow {
    ...
    public Ev3C4ev3CompilerWorkflow(PluginProperties pluginProperties) {
        ...
        this.tempDir = pluginProperties.getTempDir();
        this.compiler = new C4Ev3SourceCompiler(compilerResourceDir);
        this.uf2Builder = new Uf2Builder(compilerResourceDir);
    }
    ...
    public void compileSourceCode(String token, String programName, ILanguage language, Object flagProvider) {
        ...
        boolean compilationSuccess = this.compiler.compile(sourceCodeFileName, binaryFileName);
    ...
    private String getSourceCodeFileName(String token, String programName) {
        return this.tempDir + token + "/" + programName + "/source/" + programName + GENERATED_SOURCE_CODE_EXTENSION;
    }

    private String getBinaryName(String token, String programName) {
        return this.tempDir + token + "/" + programName + "/target/" + programName + GENERATED_BINARY_EXTENSION;
    }

    private boolean createUf2File(String programName, String binaryFileName) {
        try {
            Uf2FileContainer uf2 = this.uf2Builder.createUf2File(programName, binaryFileName);
