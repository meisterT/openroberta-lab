...
public class BotnrollCompilerWorkflow extends AbstractCompilerWorkflow {
    ...
    public void generateSourceCode(String token, String programName, Project data, ILanguage language) {
        ...
        /*
        AbstractProgramValidatorVisitor programValidator = new ArduinoBrickValidatorVisitor(data.getRobotConfiguration());
        programValidator.check(data.getTransformedProgram());
        if ( programValidator.getErrorCount() > 0 ) {
            this.workflowResult = Key.COMPILERWORKFLOW_ERROR_PROGRAM_TRANSFORM_FAILED;
            return;
        }*/
        try {
            this.generatedSourceCode = BotnrollCppVisitor.generate(data.getConfigurationAst(), data.getProgramAst().getTree(), true);
